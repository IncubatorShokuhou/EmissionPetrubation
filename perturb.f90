...
PROGRAM perturb
   ...
   INTEGER                    :: seed,nreg,nij,Nmem
   INTEGER                    :: iargc
   INTEGER                    :: ierr
   REAL(8)                    :: mean,stddev
   REAL(8)                    :: mean_c,stddev_c ! current
   REAL(8)                    :: mean_o,stddev_o ! original 
   LOGICAL                    :: LogNormal=.FALSE.!,Shift=.FALSE. 
!  INTEGER                    :: ilon,ilat
...
! .. Changed by Tie Dai for no perturb...............................................
   ...
   IF ((distribution.ne.'normal').AND.(distribution.ne.'lognormal')) THEN
      ...
      STOP 'Aborted'
   ENDIF
!   IF ((nreg.ne.1 ).AND.(nreg.ne.2 ).AND.(nreg.ne.4 ).AND.(nreg.ne.8 ).AND.&
!       (nreg.ne.16).AND.(nreg.ne.32).AND.(nreg.ne.64)) THEN
!      WRITE(*,*) 'perturb.f90'
...
! ...Create spatially correlated noise...........................................
     DO ij=1,gall,nij
        DO l=1,lall,nreg
          perturbation(ij:ij+nij-1,l:l+nreg-1,:) = pfactors(:,:,:)
        ENDDO     
     ENDDO
...
!      DO ilon=1,nLon
!         DO ilat=1,nLat
!            perturbation(MOD(ilon-1+nreg/2,nLon)+1,MOD(ilat-1+nreg/2,nLat)+1) = tmp(ilon,ilat)
!         ENDDO
!      ENDDO
!   ENDIF
   ...
   do nm=1,Nmem
      ...
      do l=1,lall
         ...
         OPEN(Unit=out, FILE=TRIM(fname),ACCESS='direct', &
                  FORM='UNFORMATTED', recl=gall*8,STATUS='REPLACE',ACTION='WRITE')
         do nlg = 1, nlag+1
            do ns = 1, nslot
               recnum=recnum+1
               WRITE(out,rec=recnum) perturbation(:,l,nm)
            end do 
         end do
         CLOSE(out)
      end do
