...
!    Please execute " ncdump -h ./origin/1level/wrfchemi_d01_2016-12-01_00:00:00.nc > output.txt " 
!-------------------------------------------------------
!     Define Variables.
...
do NNmembersNN=0,Nmembers-1,1
	   ...
	   call system ('cd ../'//Trim(NNmembersNN1)//''//Trim(NNmembersNN2)//''//Trim(NNmembersNN3)//'/; bash copy12.sh' )
  do date=1,31,1
	  do hour=0,23,1
		  call date_hour_string(date,date1,date2)
		  call date_hour_string(hour,hour1,hour2)
! Open netCDF file.
      status=nf_open('../'//Trim(NNmembersNN1)//''//Trim(NNmembersNN2)//''//Trim(NNmembersNN3)//'/wrfchemi_d01_2016-12-'//Trim(date1)//''//Trim(date2)//'_'//Trim(hour1)//''//Trim(hour2)//':00:00.nc',nf_write,ncid)
