...
! Mar 4, 2008, added gengauss routine (N.A.J. Schutgens)
! Mar 6, 2008, added mti = n + 1 to init_genrand (N.A.J. Schutgens)
...
!  public  :: genrand_real1, genrand_real2, genrand_real3, genrand_res53
  ...
  subroutine init_genrand( s )
    ...
    mtinit = .true._wi
    mti   = n + 1_wi   ! to ensure that same seed implies same 'random' series
    mt(1) = ibits( s, 0, 32 )
    do i = 2, n, 1
...
! This function generates normal random variables, based on a Box-Muller
! transform of uniform random variables
! by N.A.J. Schutgens, March 5, 2008
   FUNCTION gengauss( ndim )
	  IMPLICIT NONE
		INTEGER,INTENT(IN)  :: ndim
		REAL(KIND=wr)       :: gengauss(ndim)
		
		INTEGER             :: i
		REAL(KIND=wr)       :: rnd1,rnd2
		REAL,PARAMETER      :: pi=3.1415926536d0
		
      DO i=1,FLOOR(ndim/2.)
         rnd1 = genrand_real3()
         rnd2 = genrand_real3()
         gengauss(i*2-1) = sqrt( -2.d0 * log( rnd1 ) ) * sin( 2.d0*pi*rnd2 )
         gengauss(i*2)   = sqrt( -2.d0 * log( rnd1 ) ) * cos( 2.d0*pi*rnd2 )
      END DO
      IF( MOD(ndim,2).NE.0 ) THEN
         rnd1 = genrand_real3()
         rnd2 = genrand_real3()
         gengauss(ndim)  = sqrt( -2.d0 * log( rnd1 ) ) * sin( 2.d0*pi*rnd2 )
	    ENDIF
	 END FUNCTION gengauss
END MODULE mt19937
