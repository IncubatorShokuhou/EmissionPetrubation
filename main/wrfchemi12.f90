...
!    Please execute " ncdump -h wrfchemi_d01_2016-12-01_00:00:00.nc > output.txt " 
! -------------------------------------------------------
!     Define Variables.
...
! -------------------------------------------------------------
     ...
     REAL(8)        :: pfactors(Nmembers) ! all the grids in the file share a same pfactor(temporarily)
...
! -------------------------------------------------------------
     ...
     INTEGER*4   :: i,j,k,NNmembersNN ! member cycle
...
!       DO j=1,west_east
          ...
          seed=seed+1     ! maybe not useful (but not harmful as well) as there is no cycle here.
...
! Start to cycle according to the filename
    call system ('dir ../*/wrfchemi_d01_2016-12-01_00:00:00.nc  > 1.txt' )
    open(500,file='1.txt')
    ...
    DO i=1,west_east
	    DO j=1,south_north
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ORGC_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_SO4I_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_SO4J_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_SO4C_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NO3I_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NO3J_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NO3C_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_PM25I_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_PM25J_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_PM10_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ECI_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NH3_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ALD_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_OL2_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_HCHO_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ISO_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ETH_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_XYL_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_TOL_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_OLI_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_CH3OH_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ECJ_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_C2H5OH_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_HC3_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_HC5_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_HC8_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_OLT_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_CSL_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_KET_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ORA2_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ECC_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NO_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_NO2_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_SO2_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ORGI_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
           ...
           IF (LogNormal .EQ. .TRUE.) THEN
           	  stddev=0.0
           	  mean=1.0
              stddev = (E_ORGJ_stddev(i,j,1,1)/mean)**2 + 1.d0
              mean   = LOG(mean/SQRT(stddev))
              stddev = SQRT(LOG(stddev))
